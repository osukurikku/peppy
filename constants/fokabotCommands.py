...
def editMap(fro, chan, message):  # Edit maps ranking status ingame. // Added by cmyui :)
    ...
    if rankType == "love":
        log.rap(userID, "has {}d beatmap ({}): {} ({}).".format(rankType, mapType, beatmapData["song_name"], mapID),
                True)
        if mapType == 'set':
            msg = "{} has loved beatmap set: [https://osu.ppy.sh/s/{} {}]".format(name, beatmapData["beatmapset_id"],
